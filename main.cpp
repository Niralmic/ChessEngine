...
/*
TODO
1. превращение пешки не откатываетс€(как будет врем€) done
2. роккировка done
3. дл€ пешки написать функцию IsFieldUnderControl и ее использовать при проверке шаха done
...
int main()
{
    ...
    Chess c;
    c.Init();
    FigureTypeId promote = '0'; // идентификатор превращени€ пешки
    State state;
    string f, t;
    while(true)
    {
        ...
        else
        {
            ...
            MMove m(from, to, move(nptr));
            if(c.GetBoardField(from) != nullptr && c.GetBoardField(from)->GetType() == 'p') // обработка случа€ где пешка доходит до последней горизонтали
            {
